// content_script.js — FactLens verdict card injector with direct API calls
(function(){
  ...
  // Request queue: only 1 API call at a time, 2-second gap between calls
  const requestQueue = {
    queue: [],
    processing: false,
    lastCallTime: 0,
    ...
    async processQueue() {
      ...
      // Wait for 2-second gap from last call
      const timeSinceLastCall = Date.now() - this.lastCallTime;
      if (timeSinceLastCall < this.GAP_MS) {
        await new Promise(res => setTimeout(res, this.GAP_MS - timeSinceLastCall));
      }

      try {
        const result = await makeVerifyCall(text);
  ...
  async function makeVerifyCall(text) {
